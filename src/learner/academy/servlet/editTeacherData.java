package learner.academy.servlet;

import java.io.IOException;
import java.text.ParseException;
import java.text.SimpleDateFormat;
import java.util.Date;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import learner.academy.dao.DAO;
import learner.academy.model.Student;
import learner.academy.model.Teacher;

/**
 * Servlet implementation class editStudentData
 */
public class editTeacherData extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public editTeacherData() {
		super();
		// TODO Auto-generated constructor stub
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse
	 *      response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
		DAO dao = new DAO();
		Teacher teacher = new Teacher();
		teacher.setId(Long.parseLong(request.getParameter("roll")));
		teacher.setFirstName(request.getParameter("fname"));
		teacher.setLastName(request.getParameter("lname"));
		teacher.setAddress(request.getParameter("address"));
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		teacher.setDesignation(request.getParameter("designation"));
		Date dob = new Date();
		try {
			dob = sdf.parse(request.getParameter("dob"));
			teacher.setDob(dob);

		} catch (ParseException e) {
			e.printStackTrace();
		}
		System.out.println(teacher);
		/*
		 * Long cid = Long.parseLong(request.getParameter("class")); MyClass myclass =
		 * dao.getClass(cid); if(myclass!=null) {student.setMyclass(myclass);}
		 */
		teacher.setPhone(Long.parseLong(request.getParameter("phone")));
		boolean flag = dao.update(teacher, teacher.getId());
		System.out.println("gggggggggggg" + flag);

		if (flag) {
			response.sendRedirect("teachList.jsp");
		}
		else
		{
			response.sendRedirect("error.jsp");
		}
	}
		catch(Exception exp)
		{
			response.sendRedirect("error.jsp");

		}
	}

}
